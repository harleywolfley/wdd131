...
//filter returns a test true or false
function searchList(list, query) {

    function searchCallBack(item) {
        console.log(item)
        //.name is the string to connect to array-focusing on the object "name"
        return [
            item.name.toLowerCase().includes(query.toLowerCase())||
        // || is like or
        item.description.toLowerCase().includes(query.toLowerCase()) ||
        //item has an array called tags, referenced, then find runs through loop that takes the first tag and if it's true then tags is true and continue eval. tags is not a string, but tag is bc arrow function
...
//finds within the list
//searchList(simpleList, "an")

//searching for "" in hikes
searchList(hikes, "ea")